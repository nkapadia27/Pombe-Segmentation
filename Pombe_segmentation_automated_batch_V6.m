...
%std_thresh =1800;
%int_thresh = 
prompt_dlg = {'H factor', 'Erode Diameter', 'Dilate Diameter', 'Sensitivity Threshold', 'Min Area Threshold','Eccentricity Threshold','Folder Name','Filename'};
dlg_title = 'Parameters';
dim_box = [1,50];
definput = {'4','1','3','0.70','400','0.70','Segmented Files','Cdc13_seg'};
user_input_seg = inputdlg(prompt_dlg,dlg_title,dim_box,definput);
folder_name = user_input_seg{7};
...
min_size = str2num(user_input_seg{5});%default 400
adapt_sens_thresh = str2num(user_input_seg{4});%default 0.7%default 0.70
filename_name = user_input_seg{8};
...
big_thresh = bwareafilt(big_thresh, [min_size,100000]);
...
bw4 = bw2;
bw4(Ld2 == 0) = 0;
bw4 = bwareafilt(bw4, [min_size, 50000]);
%figure,imshow(bw4)
bw5=bw4;
...
D3 = imimposemin(D2,mask2);
 Ld3= watershed(D3);
   ...
   bw6(Ld3 == 0) = 0;
    ...
    bw6 = bwareafilt(bw6, [min_size, 100000]); 
  %figure,imshow(bw6)
fname1 = sprintf(strcat(folder_name,'/',filename_name_save), i);

cc_bin = bwconncomp(bw6);
label_bin = labelmatrix(cc_bin);
   rgb_I4 = label2rgb(label_bin, 'jet','w','shuffle');
imwrite(uint16(bw6),strcat(folder_name,'/',filename_name_save),'WriteMode','append');
